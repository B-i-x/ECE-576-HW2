    ...
    void compute_sad()
    {
        ...
        for (int block = 0; block < NUM_BLOCKS; block++)
        {
            ...
            for (int i = 0; i < BLOCK_SIZE; i++)
            {
                ...
                if(i<BLOCK_SIZE) wait(3*10, SC_NS); //3 op : comparison of 'i', assignment AND increment of 'i' (loop)
            }
            ...
            //if statement used just for delay logic, this is delay of for loop
            if(block<NUM_BLOCKS) wait(3*10, SC_NS); //3 op : comparison of 'block', assignment AND increment of 'block' (loop)
        }
